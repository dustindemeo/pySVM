...
def skSVM(X, y, scoring, tuned_parameters):
    ...
    print "clf.score(X_test, y_test) = {0}%".format(int((clf_array[0][1].score(X_test, y_test) * 10000))/100.)
    print

    clf_array[1][1].fit(X_train, y_train)
    print "clf.get_params(deep=True) =", clf_array[1][1].get_params(deep=True)
    print "clf.score(X_test, y_test) = {0}%".format(int((clf_array[1][1].score(X_test, y_test) * 10000))/100.)
    print

    clf_array[2][1].fit(X_train, y_train)
    print "clf.get_params(deep=True) =", clf_array[2][1].get_params(deep=True)
    print "clf.score(X_test, y_test) = {0}%".format(int((clf_array[2][1].score(X_test, y_test) * 10000))/100.)
    print 
